...
const ContactGroupe: FC = () => {
  ...
  const { data: contactListsResponse, isLoading: isLoadingContactLists } = useGetAllContactListsByUserIdQuery(userId)

  const { data: contactsInList } = useGetAllContactsByContactListIDQuery(
    selectedContactListId,
    { skip: selectedContactListId === '' || showAllContacts }
  )

  const { data: allUserContacts } = useGetAllContactsByUserIdQuery(
    userId,
